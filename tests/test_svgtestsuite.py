...
from os.path import splitext, join, basename, normpath, abspath
import unittest
import string
...
def get_file_list(path, pattern, skip_list=[]):
    ...
    for filename_full in full_filelist:
        os.path.basename(filename_full)
        # is the file in the skip list?
        if filename_full not in files_to_skip:
...
class SVGListTestCase(unittest.TestCase):
    ...
    def test_convert(self):
        for svgfile in self.svglist:
            try:
                tikz_code = tkzex.convert_file(
                    svgfile, crop=True, ignore_text=True, verbose=True
                )
            except:
            ...
            tex_fn = join(self.texdir, basename(splitext(svgfile)[0]) + ".tex")
            #
        ...
        for fn in self.compiled_files:
            ...
            png_fn = os.path.realpath(
                normpath(join(self.pngdir, "" + basename(splitext(fn)[0]) + ".png"))
            ).replace("\\", "/")
            pdffile = normpath(splitext(fn)[0]).replace("\\", "/")
            testfile = basename(splitext(fn)[0]) + ".svg"
            s += fig_template.substitute(
                pngfile=png_fn, pdffile=pdffile, testfile=testfile
            )
